...
文件头示例：
    ...
    ; CHIP: HK64S825
    ; 工具链：builtin_compiler；编译 release
    ; 硬件阶段：未要求，仅作为可选后续
...
- 业务掩码、延时计数、尺寸和枚举若定义为 `EQU`，源码必须真正引用该符号；未使用定义应删除，不得继续散落同值魔数形成两个来源。
...
交付 ASM 的说明性注释使用中文，写“为什么”和硬件契约，不重复显而易见的机器动作。寄存器名、指令名、标签和协议专有名词可保留原文。
...
- [ ] 业务 `EQU` 均被真实引用；不存在定义后又散落同值魔数的双来源。
- [ ] 所有子程序有 clobber 契约。
- [ ] skip 指令下一条语义明确。
- [ ] ORG 区间无重叠、无无意义大空洞。
- [ ] warnings 为 0。
- [ ] 含 DB 时未使用 `python_source_module_cli`；默认由 `builtin_compiler` 构建并按查表规范验收。
- [ ] 说明性注释为中文。
