...
本文件用于任何生成、修改、审查、构建或诊断 HK64S825 ASM 的 AI 智能体。`HK64S8101` 是已确认等同于 `HK64S825` 的型号别名，可按同一规则处理。除非上级系统指令另有规定，以下 `MUST` 规则具有交付阻断效力。
...
目标芯片确认时，`HK64S825` 和已确认别名 `HK64S8101` 都视为本规范支持；其他型号不得猜测兼容。
...
- ACK 采样必须读 `PB_INS`，不得读 `PB_PIO`；PB_PIO 可能是输出锁存，不能证明真实引脚 ACK。
- 第一条 OLED 命令或 I2C 数据事务前必须有上电稳定延时。
