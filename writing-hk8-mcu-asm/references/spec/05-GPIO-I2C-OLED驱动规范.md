...
简单 LED/GPIO 任务先判断当前功能真正需要的电气属性，不要从模板惯性写完整端口初始化序列。若只是推挽输出一个或多个 LED，默认只需要：

1. 用 `PIO` 预装安全输出电平，避免打开输出瞬间毛刺。
2. 用 `POE` 打开目标 bit 的输出使能。
3. 用 read-modify-write 保留同一端口上不属于本任务的 bit。

只有 board profile 明确要求上拉、下拉、开漏、输入通道或特殊功能选择时，才写 `PPU/PPD/POD/INS/IOS/PSL` 中对应寄存器。不得为了“初始化完整”而清写整套寄存器；这会让简单 LED 代码变重，也可能覆盖其他模块的端口配置。
...
当前 PB6/PB7 I2C/OLED 基线必须显式处理上拉、下拉、开漏、输入采样、idle latch 和输出使能。先预装 `PB_PIO=0xC0`，最后再 `PB_POE=0xC0`，避免输出打开瞬间产生错误 START/STOP 或低脉冲。

```asm
MOV A,#0x00
MOV PB_PPD,A           ; PB6/PB7 禁用下拉
MOV PB_PSL,A           ; PB6/PB7 选择 GPIO 功能
MOV A,#0xC0
MOV PB_PPU,A           ; PB6/PB7 内部上拉；外部上拉仍按原理图确认
MOV A,#0xC0
MOV PB_POD,A           ; PB6/PB7 开漏输出
MOV A,#0xC0
MOV PB_INS,A           ; PB7 采样 ACK 需要输入通道
MOV A,#0xC0
