...
先采用资料库中已经确定的 HK64S825 芯片级规则。板级规则只有在用户直接提供或明确选择对应 board profile 后才能采用；不得因请求未提开发板而选用当前板。PinContract 只在任务使用 GPIO 时要求；下表字段也只有会改变当前代码、电气安全或协议行为时才成为缺口：
...
OLED 查表任务在候选生成前必须解析芯片型号、主频、MTP 容量、OLED 分辨率、I2C 地址、SDA/SCL 引脚、上拉/开漏方式、显示方向和是否反色。用户明确选择的 profile 或用户请求已经明确的参数直接采用，不得重复询问；未确认 profile 时不得从规范中的注册板资料取值，缺口必须先按选择题确认。

### OLED/I2C 电气问题必须前置

即使 SDA/SCL 引脚映射已由用户确认的 profile 确定，创建候选源码前仍必须逐引脚确认 SDA、SCL 是否设置对应端口的 `POD`，并确认上拉来自外部电阻、内部 PPU 或两者。用户已明确给出某一项时不得重复询问；未明确时使用 Skill 规定的 A/B/C/D 选择题。禁止先写源码或运行门禁，再因 `POD`、上拉来源不明而中止。
...
```text
BDF glyph (Unicode) -> baseline canvas -> ssd1306-page-lsb-top source bytes
-> 已验证方向 profile -> manifest -> DB + TABL/TABH
```
...
`u8g2_font_*` 是 U8g2 专有 RLE 流，不是 SSD1306 page 数据，禁止直接复制到 `DB`。
