...
当前项目 E1 profile 仅适用于同一开发板；`HK64S8101` 是已确认等同于 `HK64S825` 的型号别名：
...
当前 PB6/PB7 I2C/OLED 默认采用已验证最小初始化。OLED 亮屏首版先只建立通信所需的三件事：`PB_PPU` 上拉、`PB_POE` 输出使能、`PB_PIO` 把 SDA/SCL 置为 idle high。
