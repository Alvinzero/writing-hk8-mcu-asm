...
- [ ] OLED controller 来自用户输入或用户明确选择的 `oled.json`，没有采用模块默认控制器。
...
- [ ] 只有用户明确选择 `HK64S825-4DIGIT-MIXED-PA-PB-E1` 时才使用 `hk64s825-4digit-mixed-pa-pb-e1-a1-c0-page-lsb-top-v1`；其他板或源格式已建立各自的 E1 profile。
- [ ] 只有用户明确选择 `HK64S825-SH1106-1P3-I2C-PB6-PB7-E1` 时才使用对应 SH1106 profile；已按 `B0H..B7H`、`+2` 列偏移和 `02H/10H` 起始列规划逐页发送，没有复用 SSD1306 window。
- [ ] 当前 SH1106 profile 的 I2C shift 使用 `RLR`，GDDRAM 写完后发送 `AFH`。
