...
- 当前已验证显示基线为 SSD1306 128x64、7-bit 地址 `3CH`、写地址 `78H`、PB7=SDA、PB6=SCL、命令模式控制字节 `00H`、数据模式控制字节 `40H`、正常显示命令 `A6H`。当前板反馈证明 `A0H + C0H` 只交换两个汉字的位置，不能修正单字内部镜像；最新实板照片进一步证明 `A1H + C0H` 配合单字列逆序仍会造成单字镜像。当前修正候选必须使用 `A1H + C0H` 配合原始 page 列顺序，实板复验前只能标为候选，不得宣称已验证；换板时必须重新确认显示方向。
