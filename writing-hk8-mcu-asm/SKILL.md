...
本 Skill 面向公司唯一 8 位 MCU `HK64S825`。目标是按 `references/spec/` 的规则重新撰写 ASM，并在静态检查和目标编译通过后即可 release。烧录、回读、逻辑分析仪或其他实板验证暂不作为输出 ASM 的前置条件；如用户另行要求，再作为后续验证单独执行和标注。若本机真实编译 profile/config 尚未就绪，只能进入“未编译草案”降级路径，不得声称编译通过或 release。
...
3. 若真实 profile/config 已就绪，根据规则新写候选 ASM 到隔离运行目录，不向用户展示候选源码。
4. 运行静态检查和目标编译。
5. 只有 `release` 返回 `RELEASED` 后，才输出已编译 ASM 和编译凭据。
6. 若失败原因仅为 `PROFILE_NOT_READY` 或本机工具链配置占位符，按“未编译草案模式”处理，不要把它伪装成 release。
...
PROFILE_NOT_READY 只阻止 release，不阻止未编译草案。若用户已确认芯片、功能、必要板级选项，并且用户已明确选择“先给草案”，或当前任务只要求审阅/讨论代码思路，可以直接给出 ASM 草案；但必须把标题和说明写成“未编译草案”。

进入草案模式前，若用户还没选择，按 A/B/C 询问：

```text
当前电脑缺少真实编译 profile/config，不能 release。请选择：
A. 先配置工具链后再输出编译通过 ASM（推荐）
B. 先给未编译草案，仅用于审阅，之后再补编译
C. 暂停
```

草案输出必须显式标注：未编译、未 release、不可烧录、不可量产。不得把草案称为正确 ASM、编译通过、已验证或 release；不得伪造 run ID、source hash、artifact hash 或 evidence hash；不得声称经过真实 ASMC、静态检查、烧录、回读或实板验证。真实 profile/config 就绪后，必须重新执行 `doctor -> new-run -> close-loop -> release`，再交付编译通过版本。
...
- release 模式下，候选 ASM 在 release 前只能存在于隔离运行目录中；未编译草案模式必须显式标注降级状态。
...
- release 门禁失败时，只返回诊断和 evidence 路径，不得展示 release 候选 ASM；若失败原因仅为本机 profile/config 未就绪，且用户选择草案模式，可展示未编译草案。
