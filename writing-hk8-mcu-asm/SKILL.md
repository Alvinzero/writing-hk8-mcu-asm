...
- 当前已验证显示基线为 SSD1306 128x64、7-bit 地址 `3CH`、写地址 `78H`、PB7=SDA、PB6=SCL、命令模式控制字节 `00H`、数据模式控制字节 `40H`、正常显示命令 `A6H`。当前 board profile 使用 `A1H + C0H` 配合原始 page 列顺序；换板时必须重新确认显示方向。
- 当前板 5x7 ASCII 数字/斜杠已用 `2026/7/24` 实板验证：字符按文本顺序发送，每个字符保持标准 5 列加 1 空列的原始列顺序；标准 5x7 列字节必须先做 bit 顺序反转，再作为 SSD1306 page byte 发送。不要把 bit 反转和逐字符列逆序同时当成默认修复。
- 排查方向错误时，分别判断控制器整屏列/行映射、同一行中字块排列顺序、单个字模内部列方向，不得把三者混为一次整行翻转。使用左右和上下均不对称的测试图，每次只改变一个变量并记录实板结果；字块位置正确但每个字块左右镜像时，不得交换字块顺序。
- 控制器 segment remap 与软件逐字列逆序都会改变水平方向；未经当前板实验证据不得同时叠加。若叠加后仍为单字镜像，应撤销软件列逆序并保留字模生成器输出的原始 page 列顺序。任何新组合在实板复验前只能标为候选，不得宣称已验证。
