...
description: 用于生成、修改、审查或编译公司 HK64S825 8 位 MCU 的 ASM；当 ASM 请求涉及非 HK64S825 或不在本 Skill 资料库支持范围内的芯片时，必须触发并拒绝编写、修改、审查或输出任何 ASM 代码。适用于芯片专属汇编、LED/OLED/数码管功能、静态检查、内置编译模块编译通过后输出 ASM 或失败关闭交付时。
---
...
- 本 Skill 只允许处理 `HK64S825` ASM。任何明确或可判断为其他芯片、其他 MCU、其他架构，或未在本 Skill `references/spec/` 支持范围内的型号，必须 fail closed：拒绝编写、修改、补全、转换、审查并拒绝输出 ASM 代码、伪代码、片段、模板、指令序列、寄存器推测或兼容性建议。
- 对非 `HK64S825` 请求，只说明“本 Skill 仅支持 HK64S825，不能编写或输出其他芯片 ASM。”可提示用户提供对应芯片资料或使用对应 Skill；不得提供任何可直接用于目标芯片的 ASM 内容。
- 若请求混合 `HK64S825` 与其他芯片，只处理 `HK64S825` 部分；其他芯片部分按上述规则拒绝，不得为了迁移、对照、改写或教学目的输出其他芯片 ASM。
- 对型号模糊、代号疑似其他芯片，或用户要求“照着某芯片”但最终目标未确认 `HK64S825` 的请求，先按“第一条回复”确认型号；确认不是 `HK64S825` 即停止。
...
- 若用户请求明确写出其他芯片型号，立即停止并说明暂不支持；不得猜测架构、寄存器或指令集，不得编写、修改、审查或输出该芯片 ASM、片段、模板或伪代码。
...
- 为保留已实板验证的数字/图形，可用 `bdf_to_ssd1306.py --base-manifest ... --replace-label 年 --replace-label 月 --replace-label 号` 仅替换指定字块。此模式必须保持原 layout、字符顺序、窗口、数字字模和已验证方向 profile 不变。
