...
description: 用于生成、修改、审查或编译公司 HK64S825/HK64S8101 8 位 MCU 的 ASM，适用于芯片专属汇编、LED/OLED/数码管功能、静态检查、内置编译模块编译通过后输出 ASM 或失败关闭交付时。
---

# HK64S825 ASM 编译闭环 Skill

本 Skill 面向公司唯一 8 位 MCU `HK64S825`；`HK64S8101` 是已确认等同于 `HK64S825` 的型号别名。默认使用 Skill 内置 HK64S825 编译模块完成静态检查和目标编译，通过后即可 release；烧录、回读、逻辑分析仪或其他实板验证暂不作为输出 ASM 的前置条件。失败时只返回诊断，不展示候选 ASM。

## 第一条回复

每次调用本 Skill 后，第一条回复必须先询问并确认芯片型号，不得从上下文猜测：

```text
请先确认目标芯片型号是否为 HK64S825？如果板上丝印/项目名是 HK64S8101，也可按 HK64S825 继续。
```

如果用户确认的型号既不是 `HK64S825` 也不是已确认别名 `HK64S8101`，立即停止并说明暂不支持。选择 `HK64S825` 或 `HK64S8101` 后，都按 HK64S825 使用 `references/spec/` 中的芯片规则、指令集、寄存器、内存、程序布局、LED、OLED 和数码管规范来设计 ASM，不得追问与当前功能无关的输入。
...
- 目标芯片是否为 `HK64S825` 或已确认别名 `HK64S8101`；
...
生成 OLED/SSD1306 ASM 时，读取本 Skill 的 `05-GPIO-I2C-OLED驱动规范.md`。项目经验、旧示例和模板冲突时，以当前 `HK64S825/HK64S8101` 目标、已编译证据和实板验证结论为准；带其他旧芯片型号的文件只能作为反例或历史线索，不得作为候选源码模板。

至少保证：

- 目标芯片为 `HK64S825` 或 `HK64S8101`，源码注释要说明 `HK64S8101` 按 HK64S825 别名处理。
